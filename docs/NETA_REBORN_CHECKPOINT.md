...
- JUNO to NETA was confirmed live in transaction `217B5B1FFBB1F0AC32E7299907EEF334A7EDE167F34AD16C9831119D80B4FB0D`.
- NETA to JUNO was confirmed live in transaction `7039CE5A59700E01971ACB04FC8257FDAAAE7EA0EB431B857C72F5F5AA869460` at height `41751330`, chain code `0`. The CW20 hook sent `10000` raw NETA and the Pair returned `984768 ujuno`, above the displayed `935529 ujuno` minimum.
- Both native-JUNO execution and the NETA CW20 send-hook path therefore passed live inclusion, receiving-event and minimum-received verification.
...
\n- Later Uni-7 chain inspection showed the current audited uploads at code ID `109` (stake mock, SHA-256 `c4920d17...30f81`) and code ID `110` (Socials, SHA-256 `49da22c2...ee869`), both created by the deployment wallet. The earlier code-101 checkpoint belonged to a prior chain state and is superseded. No instantiated contracts were returned for either current code at this checkpoint. The console must therefore reuse 109/110, discard unverified locally cached contract addresses after reload, instantiate the stake mock first, and only then instantiate Socials against the verified on-chain mock address.\n
...
- The Socials contract CI consumes the committed Cargo lockfile with `--locked` for schema generation, clippy, tests and release builds. A pinned `cargo-audit` release checks that exact dependency graph against the current RustSec advisory database and fails on every new advisory or warning.
- Two reviewed RustSec IDs are narrowly allowlisted in `contracts/neta-socials/.cargo/audit.toml`: RUSTSEC-2024-0344 is reachable only through CosmWasm's explicitly non-WASM `cosmwasm-crypto` dependency and is absent from the deployed artifact; RUSTSEC-2024-0388 marks CosmWasm 1.x's transitive `derivative` helper as unmaintained and has no compatible patched release. These are documented exceptions, not a global warning bypass.

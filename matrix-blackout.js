...
if(reduced)return;
let enabled=true,paused=false,running=false,timer=0,frame=0;
...
function cleanup(){cancelAnimationFrame(frame);document.body.classList.remove("neta-matrix-active","neta-blackout-active");ctx.clearRect(0,0,innerWidth,innerHeight);running=false;schedule()}
function run(){
 ...
 setTimeout(()=>{cancelAnimationFrame(frame);document.body.classList.remove("neta-matrix-active");document.body.classList.add("neta-blackout-active");setTimeout(cleanup,BLACKOUT_DURATION)},RAIN_DURATION)
}

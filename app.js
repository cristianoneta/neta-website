...
const f=n=>new Intl.NumberFormat("en-US",{maximumFractionDigits:6}).format(n);
document.querySelector("#stats").innerHTML=[
["TOTAL SUPPLY",M.stats.total_supply_neta],["TOTAL HOLDERS (UNIQUE)",M.stats.economic_master_entries],
["JUNO HOLDERS",M.stats.juno_custody_addresses],["OSMOSIS HOLDERS",M.stats.osmosis_primary_state_addresses]]
...
document.querySelector("#rows").innerHTML=H.slice(0,100).map(h=>`<tr><td>${h.rank}</td><td class="address">${h.label?`<span class="label">${h.label}</span><br>`:""}${sh(h.juno_address||h.osmosis_address)}</td><td>${f(h.juno_neta)}</td><td>${f(h.osmosis_neta)}</td><td><b>${f(h.total_neta)}</b></td><td><span class="badge ${h.type}">${h.type.replaceAll("_"," ")}</span></td></tr>`).join("");
...
function lookup(){
  ...
  const total=M.stats.economic_master_entries;
  const pct=(h.rank/total)*100;
  const topText=topPercentText(h.rank,total);
  const supplyShare=(h.total_neta/M.stats.total_supply_neta)*100;
  ...
  r.innerHTML=`<div><div class="rank">RANK #${h.rank}</div><div class="small">${h.label||h.type}</div></div>
    ...
    <div class="rstat"><div class="small">JUNO</div><div class="big">${f(h.juno_neta)}</div></div>
    <div class="rstat"><div class="small">OSMOSIS</div><div class="big">${f(h.osmosis_neta)}</div></div>`;
  ...
  panel.innerHTML=`<div class="rank-result-title">YOUR POSITION</div>
    ...
    <div class="share-line">Ranked #${h.rank} of ${f(total)} entries in the current snapshot.</div>`;
}
document.querySelector("#go").onclick=lookup;document.querySelector("#q").onkeydown=e=>{if(e.key==="Enter")lookup()};
// Matrix rain canvas

...
def main():
    ...
    if new:
        ...
        for event in new:
            ...
            event.update({"usd_value": str(usd.quantize(Decimal("0.000001"))), "valuation_method": method, "price_source": source, "price_timestamp": price_time})
    ...
    print(json.dumps({"status": output["status"], "last_height": latest, "new_tagged_events": len(new), "total_events": len(events)}, indent=2))
